...
export var RxReplicationState = function () {
    ...
    RxReplicationState.prototype.setPouchEventEmitter = function setPouchEventEmitter(evEmitter) {
        ...
        // change
        this._subs.push(Observable.fromEvent(evEmitter, 'change').subscribe(function (ev) {
        ...
        // error
        this._subs.push(Observable.fromEvent(evEmitter, 'error').subscribe(function (ev) {
            return _this2._subjects.error.next(ev);
        }));

        // active
        this._subs.push(Observable.fromEvent(evEmitter, 'active').subscribe(function () {
            return _this2._subjects.active.next(true);
        }));
        this._subs.push(Observable.fromEvent(evEmitter, 'paused').subscribe(function () {
            return _this2._subjects.active.next(false);
        }));

        // complete
        this._subs.push(Observable.fromEvent(evEmitter, 'complete').subscribe(function (info) {
...
export function watchForChanges() {
     ...
     */
    var sendChanges = {};
    var pouch$ = Observable.fromEvent(this.pouch.changes({
        since: 'now',
        live: true,
        include_docs: true
